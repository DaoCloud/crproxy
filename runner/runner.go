...
func (r *Runner) sync(ctx context.Context, lease string) {
	...
	go func() {
		defer wg.Done()
		r.runBlobSync(ctx, lease)
	}()
	for i := 0; i != 2; i++ {
		go func() {
			defer wg.Done()
			r.runManifestSync(ctx, lease)
		}()
	}
	wg.Wait()
}

func (r *Runner) runBlobSync(ctx context.Context, lease string) {
	for {
		err := r.runOnceBlobSync(context.Background(), lease)
		if err != nil {
			if err != errWait {
...
func (r *Runner) runManifestSync(ctx context.Context, lease string) {
	for {
		err := r.runOnceManifestSync(context.Background(), lease)
		if err != nil {
			if err != errWait {
...
func (r *Runner) runOnceBlobSync(ctx context.Context, lease string) error {
	var (
		err  error
		errs []error
	...
	for _, msg := range pending {
		resp, err = r.queueClient.Consume(ctx, msg.MessageID, lease)
		if err != nil {
	...
	return r.blobSync(ctx, lease, resp)
}

func (r *Runner) runOnceManifestSync(ctx context.Context, lease string) error {
	var (
		err  error
		errs []error
		resp client.MessageResponse
	...
	return r.manifestSync(ctx, lease, resp)
}
...
func (r *Runner) blobSync(ctx context.Context, lease string, resp client.MessageResponse) error {
	...
	return r.heartbeat(ctx, lease, resp.MessageID, &gotSize, &progress, errCh)
}

func (r *Runner) manifestSync(ctx context.Context, lease string, resp client.MessageResponse) error {
	...
	var gotSize, progress atomic.Int64
	go func() {
		errCh <- r.manifest(ctx, host, image, tagOrBlob, resp.Priority, &gotSize, &progress)
	}()

	return r.heartbeat(ctx, lease, resp.MessageID, &gotSize, &progress, errCh)
}

func (r *Runner) heartbeat(ctx context.Context, lease string, messageID int64, gotSize, progress *atomic.Int64, errCh chan error) error {
...
func (r *Runner) manifest(ctx context.Context, host, image, tagOrBlob string, priority int, gotSize, progress *atomic.Int64) error {
	var subCaches []*cache.Cache
	for _, cache := range r.caches {
		exist, err := cache.StatManifest(ctx, host, image, tagOrBlob)
		if err == nil {
			if exist {
				continue
			}
