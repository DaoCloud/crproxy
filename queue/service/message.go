...
func (s *MessageService) Heartbeat(ctx context.Context, id int64, lastHeartbeat time.Time, data model.MessageAttr, lease string) error {
	ctx = dao.WithDB(ctx, s.db)
	rowsAffected, err := s.messageDao.SetHeartbeatAndData(ctx, id, lastHeartbeat, data, lease)
	if err != nil {
		return err
	}
	if rowsAffected == 0 {
