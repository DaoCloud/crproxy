...
func (mc *MessageController) Create(req *restful.Request, resp *restful.Response) {
	...
	curr, err := mc.messageService.GetByID(req.Request.Context(), id)
	if err != nil {
		resp.WriteHeaderAndEntity(http.StatusNotFound, Error{Code: "MessageNotFoundError", Message: "Message not found after creation: " + err.Error()})
		return
	}
...
func (mc *MessageController) Heartbeat(req *restful.Request, resp *restful.Response) {
	...
	if err := mc.messageService.Heartbeat(req.Request.Context(), messageID, now, heartbeatRequest.Data, heartbeatRequest.Lease); err != nil {
		resp.WriteHeaderAndEntity(http.StatusNotAcceptable, Error{Code: "MessageNotAcceptableError", Message: "Message not found: " + err.Error()})
		return
	}
