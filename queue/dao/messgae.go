...
SELECT id, content, lease, priority, status, data, last_heartbeat FROM messages WHERE content = ? AND deleted_at IS NULL
`
...
SELECT id, content, lease, priority, status, data, last_heartbeat FROM messages WHERE id = ? AND deleted_at IS NULL
`
...
UPDATE messages SET content = ?, lease = ?, priority = ?, status = ?, data = ? WHERE id = ? AND deleted_at IS NULL
`
...
UPDATE messages SET priority = ? WHERE id = ? AND priority < ? AND deleted_at IS NULL
`
...
SELECT id, content, priority, status, data, last_heartbeat FROM messages WHERE deleted_at IS NULL
`
...
AND last_heartbeat < ? AND deleted_at IS NULL
`
...
UPDATE messages SET status = ?, lease = ? WHERE id = ? AND status = ? AND lease = ? AND deleted_at IS NULL
`
...
UPDATE messages SET last_heartbeat = ?, data = ? WHERE id = ? AND lease = ? AND status = ? AND deleted_at IS NULL
`

func (m *Message) SetHeartbeatAndData(ctx context.Context, id int64, lastHeartbeat time.Time, data model.MessageAttr, lease string) (int64, error) {
	db := GetDB(ctx)
	results, err := db.ExecContext(ctx, setHeartbeatAndDataSQL, lastHeartbeat, data, id, lease, model.StatusProcessing)
	if err != nil {
...
UPDATE messages SET status = ?, lease = ? WHERE id = ? AND lease = ? AND status = ? AND deleted_at IS NULL
`
...
const setFailedSQL = `
UPDATE messages SET status = ?, lease = ?, data = ? WHERE id = ? AND lease = ? AND status = ? AND deleted_at IS NULL
`
...
const cancelSQL = `
UPDATE messages SET lease = ?, status = ? WHERE id = ? AND lease = ? AND status = ? AND deleted_at IS NULL
`
...
UPDATE messages SET status = ?, lease = ? WHERE id = ? AND status = ? AND deleted_at IS NULL
`
